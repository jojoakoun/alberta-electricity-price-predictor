...
function getAlbertaHour(value) {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new TypeError(
      "A valid target timestamp is required.",
    );
  }

  const hourPart = new Intl.DateTimeFormat(
    "en-CA",
    {
...
function getTemporalWordingKey(
  targetTime,
  viewedAt = new Date(),
) {
  ...
  if (
    ...
    throw new TypeError(
      "Temporal wording requires valid timestamps.",
    );
  }
  ...
  const localHour = getAlbertaHour(
    targetDate,
  );
