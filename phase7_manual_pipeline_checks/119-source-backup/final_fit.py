...
def fit_regression_models(
  ...
  for source_row in metadata.to_dict(
    ...
    if not source_artifact_path.exists():
      raise FileNotFoundError(
        "Candidate regression artifact is missing: "
        f"{source_artifact_path}"
      )

    estimator = clone(
      joblib.load(source_artifact_path)
    )
...
def fit_classification_models(
  ...
  """Refit the five frozen spike classifiers."""
  ...
  for source_row in metadata.to_dict(
    ...
    if not source_artifact_path.exists():
      raise FileNotFoundError(
        "Candidate classification artifact is missing: "
        f"{source_artifact_path}"
      )
...
def activate_final_models() -> None:
  """Archive and atomically activate both final live model tasks."""
  ...
  for task in (
    ...
    if task not in tasks:
      raise ValueError(
        f"Active registry is missing task: {task}"
      )
