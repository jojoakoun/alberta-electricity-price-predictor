...
def test_regression_module_exposes_explicit_main(
  entrypoint_case: dict,
) -> None:
  module = import_module(
    entrypoint_case["module"]
  )

  assert callable(module.main)

  assert (
    module.TRAINING_DATASET_PATH
    is SHARED_TRAINING_DATASET_PATH
  )
...
def test_main_forwards_shared_training_path_without_fitting(
  ...
  modeling_config = {
    "test": "modeling-config"
  }

  validation_data = [
    sentinel.validation_row
  ]
  ...
  summary_function = entrypoint_case.get(
    "summary"
  )
  ...
  tuning_function = entrypoint_case.get(
    "tune"
  )

  if tuning_function is not None:
    tune_model = Mock(
      return_value=entrypoint_case[
        "tuning_result"
      ]
    )
  ...
  if tune_model is not None:
    tune_model.assert_called_once_with(
      sentinel.train_data
    )
